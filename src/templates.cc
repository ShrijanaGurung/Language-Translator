 ...
 * Modifications by: John Harwell Shrijana Gurung Luke Erlandson
 ******************************************************************************/
...
class MySequence {
 public:
  ...
  void set_member(int x, T value) { memblock_[x] = value; }

  T get_member(int x) {
    std::cout << "Shrijana Gurung" << '\n';
    std::cout << "Luke Erlandson" << '\n';
    return memblock_[x];
  }
